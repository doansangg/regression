...
effi = models.efficientnet_b2(pretrained=True)
effi = effi.to(device)
#summary(effi, (3, 360, 360))

# Freeze layers 1-6 in total 10 layers of Resnet50
# ct = 0
# for child in effi.children():
#   ct += 1
#   if ct < 7:
for param in effi.parameters():
    param.requires_grad = False

# Change the final layer of ResNet50 Model for Transfer Learning
fc_inputs = effi.classifier[1].in_features

effi.classifier = nn.Sequential(
...
optimizer = optim.Adam(effi.parameters(), lr=0.01)
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100,350], gamma=0.1)
trained_model, history, best_epoch = train_and_validate(effi, loss_func, optimizer, scheduler, num_epochs)
